...
{
    public class DiagnosisTypeController : Controller
    {
        ...
        [Authorize] public ViewResult Index(string sortOrder, string currentFilter = "", string searchString = "", int? pageNum = 0)
        {
            ...
            var objs = from o in db.GetDiagnosisTypeList()
                       select o;
        ...
        //
        // GET: /DiagnosisType/Delete/5

        [Authorize] public ActionResult Delete(int id)
        {
            try
            {
                db.DeleteDiagnosisType(id);
            }
            catch (DataException)
            {
